...
struct
  ...
  let threadenter ctx lval f args = failwith "TODO la threadenter"
  let threadspawn ctx lval f args fctx = failwith "TODO la threadspawn"

  let paths_as_set ctx = [ctx.local] (** Trivial, may be incorrect. *)

  let event ctx e octx =
    failwith "TODO la event"
end
...
struct
  ...
  let threadenter ctx lval f args = failwith "TODO"
  let threadspawn ctx lval f args fctx = lift_fun ctx (lift ctx) S.threadspawn ((|>) (conv fctx) % (|>) args % (|>) f % (|>) lval)
